...
class GETmodelo extends TPage {
    ...
    public function __construct()
    {
        ...
        var_dump($data->uf);
        if (!empty($data->uf)){
            $this->carregarMunicipios($data->uf);
        }
    ...
    private function carregarMunicipios($uf)
    {
        try {
            TTransaction::open('localidades');

            $criteria = new TCriteria;
            $criteria->add(new TFilter('id_estado', '=', $uf));
            $repository = new TRepository('Municipio');
            $municipios = $repository->load($criteria);

            $items = ['' => ''];
            if ($municipios) {
                foreach ($municipios as $obj) {
                    $items[$obj->id_municipio] = $obj->nome_municipio;
                }
            }
            ...
            // Atualiza o combo de municípios
            TCombo::reload('form_localidades', 'mun', $items);
        } catch (Exception $e) {
            new TMessage('error', $e->getMessage());
        }
    }
